...
function shouldEnableCrossSubDomainCookies(domain: string | undefined): domain is string {
  return Boolean(domain && !isLocalHost(domain))
}
...
function createAuth(env?: AppBindings, cf?: IncomingRequestCfProperties) {
  ...
  let cookieDomain: string | undefined = 'localhost'
  if (env) {
  ...
  const authConfig = {
    ...
    advanced: {
      ...
      ...(shouldEnableCrossSubDomainCookies(cookieDomain)
        ? {
